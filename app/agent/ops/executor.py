"""
Executor 节点：执行单个步骤
用于 Ops Agent，支持 Supabase、监控和日志查询
"""
...
async def executor(state: OpsAgentState) -> Dict[str, Any]:
    """
    执行节点：执行计划中的下一个步骤

    使用 LangGraph 的 ToolNode 自动处理工具调用
    """
    logger.info("=== Ops Executor：执行步骤 ===")
    ...
    try:
        ...
        # 构建消息
        messages = [
            SystemMessage(content="""你是一个能力强大的运维助手，负责执行具体的任务步骤。
...
- 专注于当前步骤，不要考虑其他任务"""),
        ...
        try:
            ...
            logger.error(f"执行步骤超时（{EXECUTION_TIMEOUT}秒）")
            result = f"执行超时，请减少查询范围或稍后重试"
