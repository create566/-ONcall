...
async def replanner(state: OpsAgentState) -> Dict[str, Any]:
    """
    重新规划节点：决定是继续、调整计划还是生成最终响应
    """
    logger.info("=== Ops Replanner：重新规划 ===")
    ...
    try:
        ...
        for tool in mcp_tools:
            ...
            for ops_tool in ["supabase", "cls", "monitor"]:
    ...
    if plan:
        ...
        try:
            messages = [
                ...
                ("user", f"重要提示：已执行 {len(past_steps)} 个步骤，请优先考虑是否信息已足够生成响应（respond）")
            ]
            ...
            logger.info(f"Ops Replanner 决策: {action}")
...
async def _generate_response(state: OpsAgentState, llm: ChatQwen) -> Dict[str, Any]:
    """生成最终响应"""
    logger.info("生成最终响应...")
